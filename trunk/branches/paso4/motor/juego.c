...
void Juego_comprobar_colisiones(Bola bola)
{
	SDL_Rect rectangulo_bola;
	
	/* Obtenemos las dimensiones de la bola */
	...
	else if(rectangulo_bola.x <= 0){
		Bola_invertir_velocidadX(bola);
		bola->x = 0;
	}
	else if(rectangulo_bola.x + rectangulo_bola.w >= PANTALLA_ANCHO){
		Bola_invertir_velocidadX(bola);
		bola->x = PANTALLA_ANCHO - rectangulo_bola.w;
	}
}
...
void Juego_bucle_principal(Juego juego)
{
	...
	while(!salir){
		...
		/* Colisiones */
		Juego_comprobar_colisiones(juego->bola);
